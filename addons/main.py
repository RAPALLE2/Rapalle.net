def main():
    try:
        ...
        print("Error | Could not import all preinstalled libraries. Please fix this before you can continue, maybe by reinstalling Python.")
    ...
    # Function to determine the operating system type
    def os_type():
    ...
    def output(msg_type, message):
        ...
        if msg_type.lower() == "info":
            ...
            color = Fore.GREEN
        elif msg_type.lower() == "thanks":
            color = Fore.GREEN
        elif msg_type.lower() == "commit":
            color = Fore.YELLOW
        elif msg_type.lower() == "warning":
            color = Fore.YELLOW
        elif msg_type.lower() == "system":
            color = Fore.LIGHTBLACK_EX
        elif msg_type.lower() == "last":
            color = Fore.LIGHTBLACK_EX
        else:
            color = Fore.WHITE
    ...
    # Function to execute commands over SSH using Fabric
    def ssh_commands(host, command):
        hostname = host[0]
        username = host[1]
        password = host[2]
        ...
        if result.stderr:
            output("error", f"{result.stderr.strip()}")

    def execute_via_batch(command):
        import tempfile
        if os_type() == "w":
            ...
            # Execute the sh file
            try:
                subprocess.run(['sh', sh_file_path], check=True)
            finally:
                # Delete the sh file after execution
                os.remove(sh_file_path)

    # Function to start the server
    def server_start(host, command):
        ip = host[0]
        mac = host[4]
        if ping(ip):
            output("success", f"{ip} is awake")
            return "online"
        else:
            output("system", f"{ip} is sleeping")
            if os_type() == 'w':
                return None
            elif os_type() == 'l':
                subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding='latin-1')
                output("success", f"The PC with the MAC address: {mac} woke up")
                return "offline"

    # Function to boot Raspberry Pi
    def raspi_boot(host):
        ip = host[0]
        if ping(ip):
            output("success", f"{ip} is awake")
            commands = 'echo "Hello, World!" && ls -l && pwd'
            ssh_commands(host, commands)
        else:
            output("error", f"{ip} is sleeping")
    ...
    def check_commit(information, only_one = False):
        ...
        def get_latest_commit(api_url):
            try:
                ...
                print(f'An error occurred while fetching the latest commit: {e}')
            return None, None
        ...
        while True:
            ...
            if not is_intense or only_one:
                ...
                if last_commit_sha == get_latest_commit_sha and is_first_time == 0 and is_fetched: 
                    is_intense = True
                elif (days_since_last_commit > 7):  
                    check_interval = month_old_commit_time
                elif ((((1 <= current_hour < 6) or (8 <= current_hour < 13 and current_weekday < 5)) and not is_holiday) or (is_holiday and 3 <= current_hour < 6)): 
                    check_interval = slow_time
                else: 
                    check_interval = normal_time

                if is_first_time == 2: 
                    is_first_time = 1
                elif is_first_time == 1: 
                    is_first_time = 0

            if not last_commit_sha == get_latest_commit_sha:
                last_commit_sha = get_latest_commit_sha
                try:
                    ssh_commands(information[0][1], f"cd {information[0][1][3]} && git fetch && git merge origin/{information[1][2]}")
                    execute_via_batch(f"cd /d {information[0][0][3]} && git fetch && git merge origin/{information[1][2]}") 
                except Exception as e: 
                    output("warning", f"Up to date or fatal error: {e}")
    ...
    def execute(command, host):
            ...
            while True:
                ...
                if category == 1:
                    ...
                    if session == 0:
                        ...
                        clear()
                        subprocess.run(f"start cmd /k ssh {host[1][1]}@{host[1][0]}", shell=True)
                        break
                    elif session == 2:
                        clear()
                        subprocess.run(f'"C://Program Files/RealVNC/VNC Viewer/vncviewer.exe" {host[1][0]}')
                        break
                    elif session == 3:
                        clear()
                        subprocess.run(f'"C://Program Files/FileZilla FTP Client/filezilla.exe" {host[1][1]}@{host[1][0]}:22')
                        break
                    elif session == 4:
                        clear()
                        subprocess.run('explorer "R:\"')
                        break
                    ...
                    if session == 0:
                        ...
                        clear()
                        subprocess.run(f"start cmd /k ssh {host[0][1]}@{host[0][0]}", shell=True)
                        break
                    elif session == 2:
                        clear()
                        subprocess.run(f'"C://Program Files/RealVNC/VNC Viewer/vncviewer.exe" {host[0][0]}')
                        break
                    elif session == 3:
                        clear()
                        subprocess.run(f"mstsc /v:{host[0][0]}")
                        break
                    elif session == 4:
                        clear()
                        subprocess.run(f'"C://Program Files/FileZilla FTP Client/filezilla.exe" {host[0][1]}@{host[0][0]}:22')
                        break
                    elif session == 5:
                        clear()
                        subprocess.run('explorer "Z:\"')
                        break
                    elif session == 6:
                        clear()
                        subprocess.run(f'"C://Program Files/WakeMeOnLan/WakeMeOnLan.exe" /wakeup {host[0][4]}')
                        break
                    ...
                    if session == 0:
                        ...
                        clear()
                        subprocess.run("start https://dash.cloudflare.com/e992ae02235a1763c1028ac553343276/rapalle.net/dns/records", shell=True)
                        break
                    elif session == 2:
                        clear()
                        subprocess.run("start https://playit.gg/account/tunnels", shell=True)
                        break
                    elif session == 3:
                        clear()
                        subprocess.run("start https://github.com/RAPALLE2/Rapalle.net", shell=True)
                        break
        ...
        if len(parts) >= 2:
            ...
            if category in credential:
                if action in reset: 
                    credentials(True)
                elif action in (info or help):
                    output("info", """Run "credentials reset" to reset the credentials, or "credentials list" to see them""")
                    time.sleep(10)
                elif action in list_cmd:
                    output("info", f"Listing all credentials: \n {credentials(False, True)}")
                    time.sleep(10)
                else: 
    ...
    def credentials(reset=False, return_credentials=False):
        ...
        if not credentials:
            ...
            output("system", "Loaded all credentials")
    ...
    while True: 
        try:
            ...
            if os_type_value == "w": 
                output("system", "Running on Windows")
            elif os_type_value == "l": 
                output("system", "Running on Linux")
            local_ip = get_ip()
            ...
            if os_type_value == "l":
                ...
                while not trys == 0:
                    trys -= 1
                    status = server_start(information[0][0], command=["sudo", "etherwake", information[0][0][4]])
                    if status == "online": break
                    time.sleep(60)
                break
            elif os_type_value == "w":
                if local_ip == information[0][0][0]:
                    execute_via_batch(f"start {information[0][0][3]}\\playit\\playit.exe")
                    raspi_boot(information[0][1])
                    check_commit(information, False)
                elif local_ip == information[0][2][0] or local_ip == "192.168.178.24":
                    while True:
                        stop = ["stop", "exit", "shutdown", "close", "s", "down", "close", "shut", "e", "c"]
                        request = input("Enter command to execute >> ")
                        if request in stop:
                            output("thanks", "Thank you for using software from rapalle.net. See you at the next startup.  :)")
                            break
                        else: 
                            execute(request, information[0])
                break
            else: 
                output("error", "This script is not running on either Windows or Linux.")
            break
